...
function isodatetime() {
    ...
    return new Date(time).toJSON().toString();
}
...
function wsseHeaderJSON(Username, Password) {
    var w = wsse(Password);
   // var header = 'UsernameToken Username="' + Username + '", PasswordDigest="' + w[2] + '", Created="' + w[1] + '", Nonce="' + w[0] + '"';
    
    var header = {
        "CreatedTimeStamp" : w[1],
        "ExpireTimeStamp" : new Date(new Date(w[1]).getTime() + 60000).toJSON().toString(),
        "passwordDigest" : w[2],
        "nonce" : w[0]
    }
    return header;
